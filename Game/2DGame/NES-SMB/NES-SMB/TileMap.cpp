...
bool TileMap::collisionMoveLeft(const glm::ivec2 &pos, const glm::ivec2 &size, int *posX, bool m) const
{
	int x, y0, y1;
	...
	for(int y=y0; y<=y1; y++)
	{
		if ((ncol.find(map[y*mapSize.x + x]) == ncol.end()) || m)
		{
			if ((*posX - tileSize * x - size.x <= 5) || m)
			{
